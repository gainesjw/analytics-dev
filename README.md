# Fabric analytics models and reports

A source-controlled Power BI semantic model and report with environment-aware lakehouse bindings.
...
Fabric semantic models and reports under `projects/`, with separate Azure DevOps CI and CD pipelines
...
| --- | --- | --- |
| `analytics-dev-ci` | `.azure-pipelines/ci.yml` | Validate items, generate governance manifest/dependency graph, publish `fabric-release` |
| `analytics-dev-cd` | `.azure-pipelines/cd.yml` | Promote the same successful main CI artifact through dev → test → prod |
...
The model's external `engineering-lakehouse` dependency resolves the SQL endpoint
from the configured engineering workspace for each environment. Set its three
workspace IDs and the analytics `semanticModelConnectionId` values (Fabric
connection GUIDs). Deploy engineering first and provision/load the required
`silver.dm_primary_care_shortage_areas` table before model refresh. CI/CD publishes
definitions; it does not create that table or run data transformations.

The old direct `deployment/deploy.py`, `deployment/parameter.yml`, and diagnostic
`.azure-pipelines/test.yml` entry points have been replaced. Disable any Azure
DevOps definitions still using those paths. The legacy standalone
`test_pbip_report/` fragment is outside `itemRoots` and is not deployed.
